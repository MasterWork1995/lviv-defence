...
function MiniDomeSVG({ sector }: { sector: number | null }) {
  ...
  return (
    <svg
      viewBox="0 0 80 58"
      className="h-[52px] w-[76px] flex-shrink-0"
      aria-hidden="true"
    >
      <path
        d="M 4 54 A 36 36 0 0 1 76 54"
        fill="none"
        stroke="#1a3a6e"
        strokeWidth="1.5"
        opacity="0.7"
...
function PersonIcon({ active }: { active: boolean }) {
  return (
    <svg
      viewBox="0 0 24 24"
      className={`h-[18px] w-[18px] flex-shrink-0 ${active ? "text-primary" : "text-text-muted"}`}
      fill="none"
      stroke="currentColor"
      strokeWidth={1.5}
      aria-hidden="true"
...
function BuildingIcon({ active }: { active: boolean }) {
  return (
    <svg
      viewBox="0 0 24 24"
      className={`h-[18px] w-[18px] flex-shrink-0 ${active ? "text-gold" : "text-text-muted"}`}
      fill="none"
      stroke="currentColor"
      strokeWidth={1.5}
      aria-hidden="true"
...
export function SearchPanel() {
  ...
  const toggle = (d: Donor) => toggleSelected(d.id);
              ...
              {donors.map((donor) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`min-w-0 flex-1 truncate text-[12px] font-medium ${active ? "text-text" : "text-text/80"}`}
                    >
                      {donor.name}
                    </span>
                    <span
                      className={`flex-shrink-0 text-[11px] font-bold ${active ? "text-dome-select" : "text-primary/80"}`}
                    >
                      — {fmtArea(donor.squareM2)}
                    </span>
                    <svg
                      className={`h-3.5 w-3.5 flex-shrink-0 transition-colors ${active ? "text-dome-select" : "text-text-muted group-hover:text-primary"}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                      aria-hidden="true"
        ...
        {selected && (
          <div className="flex-shrink-0 border-t border-border/60 px-4 py-3">
            <div className="mb-2 flex items-center justify-between">
              <p className="text-[8px] font-bold uppercase tracking-[0.3em] text-text-muted">
                Обраний сегмент
              </p>
              <button
                onClick={() => setDomeSelected(null)}
                className="cursor-pointer text-[9px] uppercase tracking-wider text-text-muted hover:text-primary transition-colors"
        ...
        <div className="flex flex-shrink-0 items-stretch border-t border-border/60">
          <div className="flex flex-1 flex-col items-center justify-center py-3">
            <div className="flex items-baseline gap-1">
              <span className="text-[18px] font-bold leading-none text-gold">
                {totalKm2}
              </span>
